...
#execfile('C:/Users/takashi/git/keyboards/hecomi_keyboard/right_hand/test.py')
import pcbnew
import re
...
ROWS.append([1   ,1   ,1   ,1   ,1   ,1   ,1   ,1.5 ])
ROWS.append([1   ,1   ,1   ,1   ,1   ,1   ,1   ,2.25])
ROWS.append([1   ,1   ,1   ,1   ,1   ,1   ,1.75,1   ])
ROWS.append([1   ,1   ,1   ,1   ,1   ,1   ,1   ,1   ])
...
def set_position_by_ref(ref):
    for r in range(0,ROW_NUM):
        prev=0
        offset_x=(OFFSETS[r]+1)*PITCH
        offset_y=1*PITCH
        prev=set_first_pos(ref,r,ROWS[r],offset_x,offset_y,first_ref_in_row[r],first_ref_in_row[r])
        if r<ROW_NUM-1:
            for i in range(first_ref_in_row[r]+1,first_ref_in_row[r+1]):
                prev=set_pos(ref,r,ROWS[r],offset_x,offset_y,i,first_ref_in_row[r],prev)
        else:
            for i in range(first_ref_in_row[r]+1,first_ref_in_row[r]+len(ROWS[len(ROWS)-1])):
                prev=set_pos(ref,r,ROWS[r],offset_x,offset_y,i,first_ref_in_row[r],prev)

#SW
set_position_by_ref("SW")
#DIODE
set_position_by_ref("D")
